...
from chess_clone.modeling.candidates import validate_no_leakage_fields, validate_candidate_width
...
def predict_candidate_probabilities(
    ...
    if not rows:
        return []
    class_probabilities = model.predict_proba(preprocessor.transform(rows))
    classes = list(getattr(model, "classes_", [0, 1]))
    raw = class_probabilities[:, classes.index(1)] if 1 in classes else np.zeros(len(rows))
...
def fit_global_rank_frequencies(
    train_rows: list[dict[str, object]], *, candidate_k: int = 5,
) -> dict[int, float]:
    ...
    return {rank: counts[rank] / total for rank in range(1, candidate_k + 1)}
...
def _confusion_matrix(
    ...
    width = max(5, max((int(r["actual_engine_rank"]) for r in results), default=5),
                max((int(r["predicted_engine_rank"]) for r in results), default=5))
    return {
        actual: {predicted: counts.get(predicted, 0) for predicted in map(str, range(1, width + 1))}
