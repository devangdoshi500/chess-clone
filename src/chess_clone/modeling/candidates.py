...
class CandidateDataset:
    """Width-aware dataset; legacy top_5 counters alias the selected width."""
    ...
    @property
    def inside_candidate_set_decisions(self) -> int:
        return self.inside_top_5_decisions

    @property
    def outside_candidate_set_decisions(self) -> int:
        return self.outside_top_5_decisions
...
def build_candidate_dataset(
    ...
    *,
    ...
    """Use a fixed top-K prefix; optionally emit zero-positive outside decisions.

    Defaults retain the legacy five-candidate, usable-only training dataset.
    Outside decisions may be scored at evaluation but must not enter binary
    training, which requires one chosen candidate per decision.
    """
    validate_candidate_width(candidate_k)
    ...
    for feature in sorted(
        ...
        lines = [line for line in lines if int(line["pv_rank"]) <= candidate_k]
        expected = min(candidate_k, chess.Board(str(feature["fen"])).legal_moves.count())
        if [int(line["pv_rank"]) for line in lines] != list(range(1, expected + 1)):
            raise ValueError(f"Incomplete candidate analysis for {key}: need {expected} lines")
        if len({str(line["best_move_uci"]) for line in lines}) != len(lines):
            raise ValueError(f"Duplicate candidate moves for {key}")
        ...
        if not usable and not include_outside:
            continue
        ...
        if positives != int(usable):
            raise ValueError(
                f"Decision {key} has {positives} positive candidate rows; expected {int(usable)}"
            )
...
def validate_candidate_width(candidate_k: int) -> None:
    if type(candidate_k) is not int or not 1 <= candidate_k <= 20:
        raise ValueError("candidate_k must be an integer from 1 through 20")


def select_candidate_rows(rows: list[dict[str, object]], candidate_k: int) -> list[dict[str, object]]:
    """Select a validated engine-rank prefix for inference, without using labels."""
    validate_candidate_width(candidate_k)
    selected = [r for r in rows if int(r["engine_rank"]) <= candidate_k]
    ranks = sorted(int(r["engine_rank"]) for r in selected)
    legal_counts = {int(r["legal_move_count"]) for r in rows if r.get("legal_move_count") is not None}
    if len(legal_counts) > 1:
        raise ValueError("Inconsistent legal move counts")
    expected = min(candidate_k, next(iter(legal_counts))) if legal_counts else candidate_k
    if ranks != list(range(1, expected + 1)):
        raise ValueError(f"Incomplete inference candidate set: expected {expected} ranks")
    if len({r["candidate_move_uci"] for r in selected}) != len(selected):
        raise ValueError("Duplicate inference candidate moves")
    return selected
