...
def train_personalized_ranker(
    ...
    *,
    ...
    dataset = build_candidate_dataset(feature_rows, analysis_rows, split, candidate_k=candidate_k)
    ...
    rank_frequencies = fit_global_rank_frequencies(rows_by_split["train"], candidate_k=candidate_k)
    training_decisions = [
    ...
    dataset_summary = {
        ...
        "inside_candidate_set_decisions": dataset.inside_candidate_set_decisions,
        "outside_candidate_set_decisions": dataset.outside_candidate_set_decisions,
...
def predict_one_decision(
    ...
    *,
    ...
    saved_k = manifest.get("candidate_k", 5)
    validate_candidate_width(saved_k)
    if candidate_k is not None and candidate_k != saved_k:
        raise ValueError(f"Inference K={candidate_k} differs from frozen model K={saved_k}")
    candidate_rows = select_candidate_rows(candidate_rows, saved_k)
